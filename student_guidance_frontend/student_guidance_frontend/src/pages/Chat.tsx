...
// import { ArrowLeft, Send, UserPlus, Plus } from "lucide-react";
...
//   // Safely grab and parse the user object from Auth.tsx
...
//       .then((res) => res.json())
//       .then((data) => {
//         // Guard against backend errors crashing the UI .map function
...
//                     className={`w-full p-4 rounded-lg text-left hover:bg-accent/50 transition-colors mb-2 ${selectedGroup?.id === g.id ? "bg-accent" : ""}`}
//                   >
//                     <div className="font-semibold">{g.groupName}</div>
//                     <div className="text-xs text-muted-foreground truncate">{g.techStack}</div>
//                   </button>
//                 ))}
//               </ScrollArea>
//             </div>
...
//                     <div>
//                       <h2 className="font-semibold text-lg">{selectedGroup.groupName}</h2>
//                       <p className="text-xs text-muted-foreground">Tech Stack: {selectedGroup.techStack}</p>
//                     </div>
//                     <Button variant="outline" size="sm" onClick={() => setIsMentorOpen(true)}>
//                       <UserPlus className="mr-2 h-4 w-4" /> Connect Mentor
//                     </Button>
//                   </div>
...
//                               {!isMe && <p className="text-xs font-bold text-secondary-foreground mb-1">{msg.sender?.name || "Anonymous"}</p>}
...
//       {selectedGroup && <ConnectMentorDialog open={isMentorOpen} onOpenChange={setIsMentorOpen} groupId={selectedGroup.id} />}
//     </div>
//   );
// };
...
// import { Card } from "@/components/ui/card";
// import { ScrollArea } from "@/components/ui/scroll-area";
// import { ArrowLeft, Send, UserPlus, Plus, Clock } from "lucide-react";
// import { CreateGroupDialog } from "@/components/CreateGroupDialog";
// import { ConnectMentorDialog } from "@/components/ConnectMentorDialog";
// import { toast } from "sonner";
...
//   // Helper utility to parse timestamps cleanly
...
//                         <span className="font-medium text-foreground/70">Tech Stack:</span> {selectedGroup.techStack}
//                       </p>
                      
//                       {/* 👥 Dynamic Members List Row Section */}
//                       <p className="text-xs text-muted-foreground">
//                         <span className="font-medium text-foreground/70">Members:</span>{" "}
//                         {selectedGroup.members?.map((m: any) => m.name).join(", ") || "No members assigned"}
//                       </p>
//                     </div>
//                     <Button variant="outline" size="sm" onClick={() => setIsMentorOpen(true)}>
//                       <UserPlus className="mr-2 h-4 w-4" /> Connect Mentor
...
//                               {/* 🕒 Timestamp Badge inside the message container */}
//                               <span 
//                                 className={`text-[10px] absolute bottom-1 right-2 flex items-center gap-0.5 ${
